  ...
  // Price multiplier (jack up prices)
  ...
  let targetPos = { x: (state.pos && state.pos.x) || 340, y: (state.pos && state.pos.y) || 220 };
  ...
  if(typeof PRICE_MULT === 'number' && PRICE_MULT !== 1){ shopData.forEach(s=>{ s.price = Math.max(1, Math.round((s.price||0) * PRICE_MULT)); }); }
  ...
  function applyUI(){
    ...
    const g = el('gubbySprite');
    if(state.pos){ g.style.left = (state.pos.x || 340) + 'px'; g.style.top = (state.pos.y || 220) + 'px'; }
    g.style.userSelect = 'none'; g.style.webkitUserDrag = 'none'; g.style.pointerEvents = 'auto'; g.style.zIndex = 50;
  ...
  function mergeRemoteState(remote){
    ...
    if(remote.pos){ state.pos = remote.pos; targetPos.x = state.pos.x; targetPos.y = state.pos.y; const g = el('gubbySprite'); if(g){ g.style.left = state.pos.x + 'px'; g.style.top = state.pos.y + 'px'; } }
  }
  ...
  function renderShopGrid(tier){
    ...
    shopData.filter(s=>s.tier===tier).forEach(item => {
      ...
      card.innerHTML = `<div style="display:flex;justify-content:space-between"><div><strong>${item.title}</strong><div class="small">${item.desc}</div></div><div style="text-align:right"><div class="small">${item.price} ✨</div></div></div>
  ...
  function wander(){
    ...
    const dx = (state.pos?.x || targetPos.x) - newX;
    const dy = (state.pos?.y || targetPos.y) - newY;
    const dist = Math.sqrt(dx*dx + dy*dy);
    const energyLoss = Math.max(1, Math.round(dist/60));
    targetPos.x = newX; targetPos.y = newY;
    state.energy = Math.max(0, state.energy - energyLoss);
    scheduleWrite();
  }
  setInterval(wander, 3000);

  // animate gubby smoothly (lerp)
  function animateGubby(){
    const g = el('gubbySprite');
    if(!g) return requestAnimationFrame(animateGubby);
    state.pos.x = state.pos.x || 340;
    state.pos.y = state.pos.y || 220;
    // easing towards target
    state.pos.x += (targetPos.x - state.pos.x) * 0.12;
    state.pos.y += (targetPos.y - state.pos.y) * 0.12;
    g.style.left = state.pos.x + 'px';
    g.style.top = state.pos.y + 'px';
    requestAnimationFrame(animateGubby);
  }
  requestAnimationFrame(animateGubby);
  ...
  // ================= GAME OVER UI =================
  function showGameOver(){ /* minimal modal alert for now */ alert('Game Over — Gubby is out of health. Restart locally or ask room to reset.'); }
  // ================= VARIANTS (simple render) =================
  function renderVariants(items){
    // optional: render variant list — left for expansion
  }
